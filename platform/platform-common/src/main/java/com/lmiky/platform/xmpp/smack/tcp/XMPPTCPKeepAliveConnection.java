 ...
 * XMPPTCPConnection在初始化的时候启动了PacketWriter的keepAlive线程
 ...
 * @author Matt Tucker
 */
public class XMPPTCPKeepAliveConnection extends XMPPConnection {

    private static final Logger LOGGER = Logger.getLogger(XMPPTCPKeepAliveConnection.class.getName());
     ...
     */
    public XMPPTCPKeepAliveConnection(String serviceName, CallbackHandler callbackHandler) {
     ...
     * @param serviceName the name of the XMPP server to connect to; e.g. <tt>example.com</tt>.
     */
    public XMPPTCPKeepAliveConnection(String serviceName) {
        // Create the configuration for this new connection
        super(new ConnectionConfiguration(serviceName));
    }
     ...
     */
    public XMPPTCPKeepAliveConnection(ConnectionConfiguration config) {
        super(config);
    }
     ...
     * @param callbackHandler the CallbackHandler used to prompt for the password to the keystore.
     */
    public XMPPTCPKeepAliveConnection(ConnectionConfiguration config, CallbackHandler callbackHandler) {
        super(config);
        config.setCallbackHandler(callbackHandler);
    }
