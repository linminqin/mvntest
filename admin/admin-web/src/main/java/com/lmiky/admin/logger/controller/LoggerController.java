package com.lmiky.admin.logger.controller;
...
import com.lmiky.admin.page.model.Page;
...
public class LoggerController extends ViewController<Logger> {
	...
	public LoggerController() {
		super();
		contentFormat = BundleUtils.getStringValue(Constants.PROPERTIES_KEY_ADMIN_FILE, "system.logger.view.content.format");
		logdescInclude = BundleUtils.getStringValue(Constants.PROPERTIES_KEY_ADMIN_FILE, "system.logger.view.logdesc.include");
	...
	/*
	 * (non-Javadoc)
	 * @see com.lmiky.admin.view.controller.ViewController#fillPage(com.lmiky.admin.web.page.model.Page, org.springframework.ui.ModelMap, javax.servlet.http.HttpServletRequest, java.util.List,
	...
	protected Page<Logger> fillPage(Page<Logger> page, ModelMap modelMap, HttpServletRequest request, List<PropertyFilter> propertyFilters, List<Sort> sorts, Map<String, Object> otherElements) {
		...
		for (Logger logger : items) {
			contet = contentFormat.replaceAll("@userName@", logger.getUserName()) //
					...
					.replaceAll("@ip@", logger.getIp() == null ? "" : logger.getIp()).replaceAll("@opeName@", LoggerOperator.getOpeName(logger.getOpeType()));
			if (!StringUtils.isBlank(logger.getPojoName())) {
				contet = contet.replaceAll("@pojoName@", LoggerOperator.getPojoName(logger.getPojoName()));
			} else {
