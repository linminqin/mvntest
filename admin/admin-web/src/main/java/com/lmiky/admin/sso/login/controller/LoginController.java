package com.lmiky.admin.sso.login.controller;
...
import com.lmiky.platform.util.CookieUtils;
...
public class LoginController extends BaseWebController {
	...
	public String login(ModelMap modelMap, HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			//记录日志
			LoggerOperator.save(request, null, null, sessionInfo.getUserId(), sessionInfo.getUserName(), OperateType.OPE_TYPE_LOGIN, this.getClass().getName(), null, service);
			//记录cookie
			...
			if(!StringUtils.isBlank(continuationId)) {
				...
				if(!StringUtils.isBlank(redirectUrl)) {
					...
					return "redirect:" + com.lmiky.platform.util.StringUtils.addUrlParameter(redirectUrl, Constants.HTTP_PARAM_LOGIN_CONTINUATION, continuationId);
				}
			}
			return "redirect:/";	//根目录
		} catch(Exception e) {
	...
	public String logout(ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		try {
			LoggerOperator.save(request, null, null, sessionInfo.getUserId(), sessionInfo.getUserName(), OperateType.OPE_TYPE_LOGOUT, this.getClass().getName(), null, service);
		} catch (Exception e) {
			logException(e, modelMap, request, response);
		}
